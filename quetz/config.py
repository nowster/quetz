...
class Config:
    ...
    def __init__(self, deployment_config: str = None) -> NoReturn:
        ...
        for section in self._config_map:
            if section.required or section.name in self.config:
                for entry in section.entries:
                    env_var_value = os.getenv(entry.env_var(section.name))

                    # Override the configuration files if an env variable is defined
                    # for the entry
                    if env_var_value:
                        value = entry.casted(env_var_value)
                    else:
                        value = self._get_value(entry, section.name)

                    setattr(self, entry.full_name(section.name), value)
